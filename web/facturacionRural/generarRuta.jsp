...
<html>
    ...
    <body>
        <div class="container">
            ...
            <div>
                <h3>Generación de Rutas Rurales</h3>
                <div class="row">
                    <h4 class="col-sm-4">Seleccione la fecha a generar la Ruta</h4>
                    <form action="generarRuta.jsp" method="post">
                        <div class="col-sm-2">
                            <input type="date" class="form-control" name="FecEnt" />
                        </div>
                        <div class="col-sm-1">
                            <button class="btn btn-success btn-block" name="accion" value="ver">Ver</button>
                        </div>
                    </form>
                    <div class="col-sm-2">
                        <form action="../FoliosRurales" method="post" class="col-sm-6">
                            <input class="hidden" name="FecEnt" value="<%=request.getParameter("FecEnt")%>" />
                            <button type="submit" class="btn btn-primary" name="accion" value="generarRemisionesGlobal" onclick="return confirm('Seguro que desea generar las remisiones?')">Generar Ruta</button>
                        </form>
                    </div>
                </div>
                <%
                ...
                <table class="table table-condensed table-bordered table-striped">
                    ...
                    <%
                        ResultSet rset = con.consulta("select r.F_IdReq, u.F_NomCli, DATE_FORMAT(r.F_FecEnt, '%d/%m/%Y'), r.F_ClaPro, r.F_PiezasReq from tb_unireqrurales r, tb_uniatn u where r.F_FecEnt='" + request.getParameter("FecEnt") + "' and r.F_ClaUni = u.F_ClaCli group by r.F_IdReq");
                        while (rset.next()) {
                    %>
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="col-sm-6">
                                <a class="btn btn-sm btn-primary" href="#" onclick="window.open('verRequerimentoCargado.jsp?F_IdReq=<%=rset.getString(1)%>', '', 'width=1200,height=800,left=50,top=50,toolbar=no')"><span class="glyphicon glyphicon-search"></span></a>
                            </div>
                        </td>
                    </tr>
                    <%
