...
<table border="1">
    ...
    <%        try {
            con.conectar();
            try {
                ResultSet rset = con.consulta("SELECT f.F_IdFact, f.F_ReqRur, u.F_NomCli, DATE_FORMAT(f.F_FecEnt, '%d/%m/%Y') as FecEnt, l.F_ClaPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y') as FecCad, SUM(f.F_Cant+0) as F_Cant, l.F_Ubica, f.F_IdFact, l.F_Cb, SUM(p.F_Pzs), (SUM(f.F_Cant) DIV p.F_Pzs) as Cajas, (SUM(f.F_Cant) MOD p.F_Pzs) as Resto, m.F_DesPro, f.F_Id, f.F_FecEnt FROM tb_factrurtemp f, tb_lote l, tb_uniatn u, tb_pzxcaja p, tb_medica m WHERE f.F_IdLot = l.F_IdLote AND f.F_ClaCli = u.F_ClaCli AND p.F_ClaPro = l.F_ClaPro AND m.F_ClaPro = l.F_ClaPro AND f.F_IdFact='" + request.getParameter("fol_gnkl") + "' group by f.F_IdLot order by l.F_ClaPro+0 asc;");
                while (rset.next()) {
